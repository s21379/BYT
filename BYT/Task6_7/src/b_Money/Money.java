...
public class Money implements Comparable {
	...
	public String toString() {
		String strAmount = "0";
		if (this.amount != 0) {
			String rawAmount = String.valueOf(this.amount);
			if (rawAmount.length() <= 2) {
				strAmount = "0." + rawAmount;
			} else {
				strAmount = rawAmount.substring(0, rawAmount.length() - 2) + '.' + rawAmount.substring(rawAmount.length() - 2);
			}
		}
		return strAmount + " " + currency.getName();
	}
	...
	public Boolean equals(Money other) {
		...
		Integer otherUniversalValue = other.universalValue();
		return thisUniversalValue.equals(otherUniversalValue);
	}
	...
	public Money add(Money other) {
		Integer otherValueInThisCurrency = this.getCurrency().valueInThisCurrency(other.amount, other.currency);
	 ...
	 * @return 0 if the values of the moneys are equal.
	...
	public int compareTo(Object other) {
		...
		Integer thisUniversalValue = this.universalValue();
		Integer otherUniversalValue = otherMoney.universalValue();

		return thisUniversalValue.compareTo(otherUniversalValue);
	}
}